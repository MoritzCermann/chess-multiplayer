...
public class Chessboard : MonoBehaviour
{
    ...
    [SerializeField] private float deathSpacing = 0.355f;
        ...
        if (Physics.Raycast(ray, out info, 100, LayerMask.GetMask("Tile", "Hover", "Highlight")))
        {
            ...
            if(Input.GetMouseButtonDown(0))
            {
                if (chessPieces[hitPosition.x,hitPosition.y] != null)
                {
                    ...
                    if ((chessPieces[hitPosition.x, hitPosition.y].team == 0 && isWhiteTurn && currentTeam == 0) || (chessPieces[hitPosition.x, hitPosition.y].team == 1 && !isWhiteTurn && currentTeam == 1))
                    {
                        ...
                        Debug.Log(specialMove + " special Move");
                        Debug.Log(availableMoves.Count + " available moves");
            ...
            if (currentlyDragging != null && Input.GetMouseButtonUp(0))
            {
                ...
                if (ContainsValidMove(ref availableMoves, new Vector2Int(hitPosition.x, hitPosition.y)))
                {
                    ...
                    // Net implementation
                    Debug.Log(previousPosition.x + " " + previousPosition.y + " " + hitPosition.x + " " + currentTeam);
            ...
            if(myPawn.currentX == enemyPawn.currentX)
            {
                if(myPawn.currentY == enemyPawn.currentY - 1 || myPawn.currentY == enemyPawn.currentY + 1)
                {
                    if(enemyPawn.team == 0)
                    {
                        ...
                        enemyPawn.SetPosition(new Vector3(8, yOffset, -1) + (Vector3.forward * deathSpacing) * deadWhites.Count);
                    }
                    else
                    {
                        deadBlacks.Add(enemyPawn);
                        enemyPawn.SetScale(Vector3.one * deathSize);
                        enemyPawn.SetPosition(new Vector3(-1, yOffset, 8) + (Vector3.back * deathSpacing) * deadBlacks.Count);
                    }
    ...
    private void SimulateMoveForSinglePiece(ChessPiece cp, ref List<Vector2Int> moves, ChessPiece targetKing)
    {
        ...
        for (int i = 0; i < moves.Count; i++)
        {
            ...
            // Did we simulate tht king's move
    ...
    private void MoveTo(int originalX, int originalY, int x, int y)
    {
        ...
        // Is there another pice on the target position?
        if (chessPieces[x,y] != null)
        {
            ...
            if(ocp.team == 0)
            {
                ...
                ocp.SetPosition(new Vector3(8, yOffset, -1) + (Vector3.forward * deathSpacing) * deadWhites.Count);
            }
            else
            {
                if (ocp.type == ChessPieceType.King)
                ...
                deadBlacks.Add(ocp);
                ocp.SetScale(Vector3.one * deathSize);
                ocp.SetPosition(new Vector3(-1, yOffset, 8) + (Vector3.back * deathSpacing) * deadBlacks.Count);
            }
        }
        ...
        // THIS CODE LOOKS DIFFERENT IN VIDEO
        switch (LookForCheckmate())
        {
            case 0:
                break;
            case 1:
    ...
    private Vector2Int LookupTileIndex(GameObject hitInfo)
    {
        for (int x = 0; x < TILE_COUNT_X; x++)
        {
            for(int y = 0; y < TILE_COUNT_Y; y++)
            {
                ...
                {
                    //Debug.Log($"{x} and {y} are the indexes");
    ...
    private void OnWelcomeServer(NetMessage msg, NetworkConnection cnn)
    {
        ...
        Debug.LogWarning(nw.AssignedTeam + "TEAM");
    ...
    {
        StartCoroutine(OnWelcomeClientCoroutine(msg));
    }
    private IEnumerator OnWelcomeClientCoroutine(NetMessage msg)
    {
        // Wait short time to let the server assign the team
    ...
    private void OnRematchClient(NetMessage msg)
    {
        ...
        // If both want to rematch, reset the game
